...
def convert_book(
    ...
    def convert(self, oeb, output_path, input_plugin, opts, log):
        ...
        # Extract spine order for reading-order sorting (optional)
        spine_order = None
        if get_config().get('use_spine_order', False):
            try:
                spine_order = [item.href for item in oeb.spine]
            except Exception:
                spine_order = None
        elements.extend(get_page_elements(oeb.manifest.items, spine_order))
...
def convert_item(
    ...
    cache_id = uid(
        input_path + translator.name + target_lang + merge_length + _encoding + chunking_method + 'norm_v1')
